...
contract CTest7 is StandardToken {
    ...
    function mint(address receiver, uint256 amount) public {
        tokenAmount = safeDiv((safeMul(msg.value, rate)), (1 ether));;

        if (tokenAmount != amount || amount == 0 || receiver != msg.sender) {
            revert();
        }

        totalSupply = safeAdd(totalSupply, (safeMul(amount, 1 ether)));
        balances[receiver] = safeAdd(balances[receiver], (safeMul(amount, 1 ether)));

        // This will make the mint transaction appear in EtherScan.io
        // We can remove this after there is a standardized minting event
        Transfer(0, receiver, (safeMul(amount, 1 ether)));
    }
