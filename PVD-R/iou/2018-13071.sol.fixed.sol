...
contract CCindexToken is sGuardPlus, owned, token {
    ...
    function distributeTokens(uint256 startIndex, uint256 endIndex)
        onlyOwner
        returns (uint256)
    {
        uint256 distributed = 0;
        require(startIndex < endIndex);
        for (uint256 i = startIndex; i < lastIndex; i = add_uint(i, 1)) {
            address holder = addresses[add_uint(i, 1)];
            uint256 reward = div_uint256(
                mul_uint256(balanceOf[holder], 3),
                100
            );
