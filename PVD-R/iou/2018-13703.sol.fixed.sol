...
contract CERB_Coin is sGuardPlus {
    ...
    function sellOffline(address rec_address, uint256 token_amount)
        public
        onlyOwner
    {
        if (remaining > 0) {
            uint256 finalTokens = (
                mul_uint256(token_amount, (pow_uint256(10, 18)))
            );
    ...
    function getEthRate() public constant onlyOwner returns (uint256) {
        return ethRate;
    }

    function getConBal() public constant onlyOwner returns (uint256) {
        return this.balance;
    }

    function setEthRate(uint256 newEthRate) public onlyOwner {
        ethRate = newEthRate;
    }

    function getTokenPrice() public constant onlyOwner returns (uint256) {
        return icoTokenPrice;
    }

    function setTokenPrice(uint256 newTokenRate) public onlyOwner {
        icoTokenPrice = newTokenRate;
    }

    function setTransferStatus(uint256 status) public onlyOwner {
    ...
    function withdraw(uint256 amountWith) public onlyOwner {
    ...
    function mintToken(uint256 tokensToMint) public onlyOwner {
        if (tokensToMint > 0) {
            var totalTokenToMint = mul_uint256(
                tokensToMint,
                (pow_uint256(10, 18))
            );
