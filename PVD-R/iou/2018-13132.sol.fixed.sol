...
contract SPXToken is sGuardPlus, StandardToken {
    ...
    uint256 public constant TOKEN_LIMIT = 8888888888 * (1e18);
...
contract SpadeIco {
    uint256 public constant TOKENS_FOR_SALE = 3655555558 * 1e18;
    uint256 public constant TOKENS_FOUNDATION = 1777777778 * 1e18;
    ...
    function finishIco(address foundation, address other) external teamOnly {
        ...
        uint256 amountWithFoundation = SafeMath.add(
            token.totalSupply(),
            TOKENS_FOUNDATION
        );
        if (amountWithFoundation > token.TOKEN_LIMIT()) {
            uint256 foundationToMint = token.TOKEN_LIMIT() -
                token.totalSupply();
            ...
            uint256 mintedTokens = token.totalSupply();
            uint256 remaining = token.TOKEN_LIMIT() - mintedTokens;
            if (remaining > 0) {
    ...
    function convertPresaleTokens(
        address buyer,
        uint256 tokens,
        uint256 factor,
        string txHash
    ) external icoAgentOnly returns (uint256) {
