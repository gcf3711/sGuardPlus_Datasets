...
contract MON is MintableToken {
    ...
    function MON(
        ...
        stageDataStore.push(
            StageData(
                uint128(start + period * 151),
                uint64(50 * sumMultiplayer),
                uint64(5000)
            )
        );
        stageDataStore.push(
            StageData(
                uint128(start + period * 243),
                uint64(60 * sumMultiplayer),
                uint64(3000)
            )
        );
        stageDataStore.push(
            StageData(
                uint128(start + period * 334),
                uint64(50 * sumMultiplayer),
                uint64(1666)
            )
        );
        ...
        stageDataStore.push(
            StageData(
                uint128(start + period * 548),
                uint64(65 * sumMultiplayer),
                uint64(1444)
            )
        );
        stageDataStore.push(
            StageData(
                uint128(start + period * 641),
                uint64(55 * sumMultiplayer),
                uint64(1000)
            )
        );
    }
    ...
    function() public payable {
        ...
        if (this.balance < msg.value) {
            ...
            amountOfEthBeforeBuy = this.balance.sub(msg.value);
        }
        stageMaxEthAmount =
            (uint256(currS.stageSum).mul((10**18))) /
            currS.stagePrice;
        ...
        if (currS.stageTime < _n && amountOfEthBeforeBuy < stageMaxEthAmount) {
            ...
            if (currS.stageTime > _n && this.balance < stageMaxEthAmount) {
                ...
                amountToMint = msg.value.mul(currS.stagePrice);
                status = status.mul(10) + 4;
                mintCoins(msg.sender, amountToMint);
            } else {
                if (this.balance >= stageMaxEthAmount) {
                    //we exceed stage limit
                    status = status.mul(10).add(5);
                    transferToBenef = true;
                    amountToMint =
                        (stageMaxEthAmount.sub(amountOfEthBeforeBuy)).mul((currS.stagePrice));
                    mintCoins(msg.sender, amountToMint);
                    stageIndex = stageIndex.add(1);
    ...
    uint256 public constant maxTokenSupply =
        (10**(18 - DECIMALS)) * (10**6) * 34;
