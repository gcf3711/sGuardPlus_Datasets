...
contract FuturXe is sGuardPlus, ERC20Interface, Owner {
    ...
    function transferFrom(
        ...
        if (add_uint256(balances[to], value) < balances[to]) return false;

        balances[from] = sub_uint256(balances[from], value);
        allowed[from][msg.sender] = sub_uint256(allowed[from][msg.sender], value);
