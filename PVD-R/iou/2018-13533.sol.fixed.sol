...
contract ALUXToken is sGuardPlus, owned, TokenERC20 {
    ...
    address public commissionGetter =
        0xCd8bf69ad65c5158F0cfAA599bBF90d7f4b52Bb0;
    ...
    function transfer(address _to, uint256 _value) public {
        uint256 market_value = _value * sellPrice;
        uint256 commission = (market_value * 4) / 1000;
    ...
    function withdraw(uint256 amountInWeis) public onlyOwner {
    ...
    function buy() public payable {
        ...
        uint256 market_value = amount * buyPrice;
        uint256 commission = (market_value * 4) / 1000;
        if (commission < minimumCommission) {
            commission = minimumCommission;
        }
    ...
    function sell(uint256 amount) public {
        require(!closeSell);
        address contr = this;
        uint256 market_value = amount * sellPrice;
        uint256 commission = (market_value * 4) / 1000;
        if (commission < minimumCommission) {
            commission = minimumCommission;
        }

        uint256 amount_weis = market_value + commission;
