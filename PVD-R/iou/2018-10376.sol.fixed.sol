...
contract SMT is sGuardPlus, StandardToken {
    ...
    function transferProxy(address _from, address _to, uint256 _value, uint256 _feeSmt,
        ...
        if(balances[_from] < add_uint256(_feeSmt , _value)) revert();

        uint256 nonce = nonces[_from];
        bytes32 h = keccak256(_from,_to,_value,_feeSmt,nonce);
        if(_from != ecrecover(h,_v,_r,_s)) revert();

        if(add_uint256(balances[_to] , _value) < balances[_to]
            || add_uint256(balances[msg.sender] , _feeSmt) < balances[msg.sender]) revert();
        balances[_to] = add_uint256(balances[_to] , _value);
        Transfer(_from, _to, _value);

        balances[msg.sender] = add_uint256(balances[msg.sender] , _feeSmt);
        Transfer(_from, msg.sender, _feeSmt);

        balances[_from] = sub_uint256(balances[_from] , add_uint256(_value , _feeSmt));
        nonces[_from] = add_uint256(nonce , 1);
        return true;
    }
