...
contract NGToken is sGuardPlus, ERC223 {
    ...
    uint256 private constant INITIAL_SUPPLY =
        20000000000 * (10**uint256(DECIMALS));
    ...
    function multipleTransfer(address[] _to, uint256 _value)
        public
        returns (bool success)
    {
        require(mul_uint256(_value, _to.length) > 0);
        require(balances[msg.sender] >= mul_uint256(_value, _to.length));
        balances[msg.sender] = sub_uint256(
            balances[msg.sender],
            mul_uint256(_value, _to.length)
        );
    ...
    function increaseApproval(address _spender, uint256 _addValue)
        public
        returns (bool)
    {
        allowed[msg.sender][_spender] = add_uint256(
    ...
    function decreaseApproval(address _spender, uint256 _subValue)
        public
        returns (bool)
    {
    ...
    function transfer(
        ...
        if (isContract(_to)) {
            ...
            assert(
                _to.call.value(0)(
                    bytes4(keccak256(_customFallback)),
                    msg.sender,
                    _value,
                    _data
                )
